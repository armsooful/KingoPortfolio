"""
FSS(금융감독원) 주택담보대출 상품 Fetcher

금융상품 한 눈에 API를 사용하여 주택담보대출 상품 정보를 조회합니다.
- URL: http://finlife.fss.or.kr/finlifeapi/mortgageLoanProductsSearch.json
- 응답: baseList(상품 기본정보) + optionList(담보유형/상환방식/금리유형별 금리)
- 권역코드별 순차 호출 + 다중 페이지 지원
- 주요 권역: 은행(020000), 저축은행(030300), 보험(050000)
"""
...
class FssMortgageLoanFetcher(BaseFetcher):
    """FSS 금융상품 한 눈에 - 주택담보대출 Fetcher"""

    BASE_URL = "http://finlife.fss.or.kr/finlifeapi/mortgageLoanProductsSearch.json"

    source_id = "FSS_MORTGAGE_LOAN"
    source_name = "금융감독원 금융상품 한 눈에 (주택담보대출)"
    supported_data_types = [DataType.MORTGAGE_LOAN_PRODUCT]
    ...
    def fetch(self, data_type: DataType, params: Dict[str, Any]) -> FetchResult:
        ...
        try:
            if data_type != DataType.MORTGAGE_LOAN_PRODUCT:
                return FetchResult.error_result(
    ...
    def _fetch_all_sectors(self) -> List[Dict[str, Any]]:
        """모든 권역코드를 순차 호출하여 주택담보대출 상품을 통합 수집"""
        all_products = []

        for sector_code, sector_name in SECTOR_CODES.items():
            self.logger.info(f"[FSS 주담대] 권역 '{sector_name}'({sector_code}) 조회 시작")
            try:
                products = self._fetch_all_pages(sector_code)
                self.logger.info(
                    f"[FSS 주담대] 권역 '{sector_name}'({sector_code}): {len(products)}개 상품 조회됨"
                )
                for p in products:
                ...
                self.logger.warning(
                    f"[FSS 주담대] 권역 '{sector_name}'({sector_code}) 조회 실패: {e.message}"
                )
            except Exception as e:
                self.logger.warning(
                    f"[FSS 주담대] 권역 '{sector_name}'({sector_code}) 조회 중 오류: {str(e)}"
                )

        self.logger.info(
            f"[FSS 주담대] 전체 권역 조회 완료: 총 {len(all_products)}개 상품, "
    ...
    def _fetch_all_pages(self, sector_code: str) -> List[Dict[str, Any]]:
        ...
        while True:
            products, max_page_no = self._fetch_mortgage_products(sector_code, page_no)
            all_products.extend(products)
    ...
    def _fetch_mortgage_products(self, sector_code: str, page_no: int = 1) -> Tuple[List[Dict[str, Any]], int]:
        """단일 권역/페이지에 대해 FSS API를 호출하여 주택담보대출 상품 조회"""
        api_params = {
            "auth": self.api_key,
        ...
        self.logger.info(
            "FSS 주택담보대출 API 호출 url=%s topFinGrpNo=%s pageNo=%d",
        ...
        self.logger.info(
            "FSS 주택담보대출 API 응답 (권역 %s, 페이지 %d/%d): baseList=%d건, optionList=%d건",
        ...
        for opt in option_list:
            ...
            option_map.setdefault(key, []).append({
                ...
                "lend_rate_min": self._safe_float(opt.get("lend_rate_min")),
                "lend_rate_max": self._safe_float(opt.get("lend_rate_max")),
                "lend_rate_avg": self._safe_float(opt.get("lend_rate_avg")),
            })
