    ...
    try:
        ...
        for ticker in MAJOR_TICKERS:
            ...
            if existing:
                ...
                print(f"  ✅ [{ticker}] {data['name']} 업데이트 - 가격: {data['current_price']:,}원, 섹터: {sector}")
                updated_count += 1
            else:
                # 새로 생성
                new_stock = Stock(
                    ...
                    is_active=True,
                    description=f"{data['name']} ({sector} 섹터)"
                )
                db.add(new_stock)

                print(f"  ✨ [{ticker}] {data['name']} 신규 등록 - 가격: {data['current_price']:,}원, 섹터: {sector}")
                created_count += 1
        ...
        print(f"  가격 정보: {with_price}개")
        print(f"  PBR 정보: {with_pbr}개")
